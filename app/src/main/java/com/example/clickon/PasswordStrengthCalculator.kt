...
class PasswordStrengthCalculator : TextWatcher {
    ...
    private fun calculateStrength(password: CharSequence) {
        if(password.length in 0..6){
            strengthColor.value = R.color.red
            strengthLevel.value = StrengthLevel.FRACA
        }else if(password.length in 7..9){
            if(lowerCase.value == 1 || upperCase.value == 1 || digit.value == 1 || specialChar.value == 1){
                strengthColor.value = R.color.yellow
                strengthLevel.value = StrengthLevel.MEDIO
            }
        }else if(password.length in 10..12){
            if(lowerCase.value == 1 || upperCase.value == 1 || digit.value == 1 || specialChar.value == 1){
                if(lowerCase.value == 1 && upperCase.value == 1){
                    strengthColor.value = R.color.green
                    strengthLevel.value = StrengthLevel.FORTE
                }
            }
        }else if(password.length > 12){
            if(lowerCase.value == 1 && upperCase.value == 1 && digit.value == 1 && specialChar.value == 1){
                strengthColor.value = R.color.white
                strengthLevel.value = StrengthLevel.MUITOFORTE
            }
        }
    }
