...
import com.example.clickon.databinding.RegisterAssistancesBinding
import com.example.clickon.pages.AssistancesActivity
import com.google.firebase.auth.FirebaseAuth


class RegisterAssistances : AppCompatActivity() {


    private lateinit var mAuth: FirebaseAuth
    private lateinit var binding: RegisterAssistancesBinding
    private var color: Int = R.color.red

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = RegisterAssistancesBinding.inflate(layoutInflater)
        setContentView(binding.root)

        mAuth = FirebaseAuth.getInstance()

        binding.buttonCreateCountAssistance.setOnClickListener { validateData() }

        val passwordStrengthCalculator = PasswordStrengthCalculator()
        binding.editPasswordRegisterAssistance.addTextChangedListener(passwordStrengthCalculator)
    ...
    private fun displayStrengthLevel(strengthLevel: StrengthLevel) {
        binding.buttonCreateCountAssistance.isEnabled = strengthLevel == StrengthLevel.MUITOFORTE
    ...
    private fun validateData() {
        val name = binding.editNameRegisterAssistance.text.toString().trim()
        val email = binding.editEmailLoginAssistance.text.toString().trim()
        val password = binding.editPasswordRegisterAssistance.text.toString().trim()
        val cpf = binding.editCpfAssistance.text.toString().trim()
        ...
        createCountFirebase( email, password, cpf)
    }

    private fun createCountFirebase( email: String, password: String, cpf: String) {
